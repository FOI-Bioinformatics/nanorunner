...
class TestResolveSpecies:
    ...
    def test_resolve_species_via_gtdb_api(self, tmp_path: Path) -> None:
        ...
        with patch("nanopore_simulator_v2.species.urllib.request.urlopen", side_effect=mock_urlopen):
            ref = resolve_species(
                "Escherichia coli",
    ...
    def test_resolve_species_gtdb_not_found_falls_to_ncbi(
        ...
        with patch("nanopore_simulator_v2.species.urllib.request.urlopen", side_effect=mock_urlopen):
            with patch("nanopore_simulator_v2.species.shutil.which", return_value="/usr/bin/datasets"):
                with patch("nanopore_simulator_v2.species.subprocess.run", return_value=mock_run_result):
                    ref = resolve_species(
...
class TestResolveTaxid:
    ...
    def test_resolve_taxid_success(self, tmp_path: Path) -> None:
        ...
        with patch("nanopore_simulator_v2.species.shutil.which", return_value="/usr/bin/datasets"):
            with patch("nanopore_simulator_v2.species.subprocess.run", return_value=mock_result):
                ref = resolve_taxid(
                    562,
                    cache=GenomeCache(cache_dir=tmp_path / "genomes"),
                    resolution_cache_dir=tmp_path / "resolutions",
    ...
    def test_resolve_taxid_datasets_not_installed(self, tmp_path: Path) -> None:
        with patch("nanopore_simulator_v2.species.shutil.which", return_value=None):
            result = resolve_taxid(
                562,
                cache=GenomeCache(cache_dir=tmp_path / "genomes"),
                resolution_cache_dir=tmp_path / "resolutions",
...
class TestDownloadGenome:
    ...
    def test_download_success(self, tmp_path: Path) -> None:
        ...
        with patch("nanopore_simulator_v2.species.shutil.which", return_value="/usr/bin/datasets"):
            with patch("nanopore_simulator_v2.species.subprocess.run", side_effect=run_side_effect):
                result = download_genome(ref, cache=cache)
...
class TestDetectDomain:
    ...
    def test_detect_bacteria_from_lineage(self) -> None:
        from nanopore_simulator_v2.species import _detect_domain
...
class TestPickBestAssembly:
    ...
    def test_single_candidate(self) -> None:
        from nanopore_simulator_v2.species import _pick_best_assembly
