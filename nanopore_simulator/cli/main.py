...
from ..core.profiles import get_available_profiles, create_config_from_profile, get_profile_recommendations, validate_profile_name
from ..core.adapters import get_available_adapters, validate_for_pipeline, get_compatible_pipelines
...
Examples:
  ...
  # Enhanced features require: pip install nanorunner[enhanced]
    ...
    parser.add_argument("--recommend", type=Path, metavar="SOURCE_DIR",
                       help="Get profile recommendations for a source directory")
    parser.add_argument("--validate-pipeline", nargs=2, metavar=("PIPELINE", "TARGET_DIR"),
    ...
    parser.add_argument("--timing-model", choices=["uniform", "random", "poisson", "adaptive"],
                       help="Timing model to use (overrides profile setting)")
    parser.add_argument("--burst-probability", type=float,
                       help="Burst probability for Poisson model (0.0-1.0)")
    parser.add_argument("--burst-rate-multiplier", type=float,
                       help="Burst rate multiplier for Poisson model")
    
    # Advanced timing options
    parser.add_argument("--random-factor", type=float,
    ...
    # Monitoring options
    parser.add_argument("--monitor", choices=["default", "detailed", "enhanced", "none"], default="default",
    ...
    # Validate arguments
    if args.random_factor is not None and (args.random_factor < 0.0 or args.random_factor > 1.0):
        parser.error("Random factor must be between 0.0 and 1.0")
    
    if args.burst_probability is not None and (args.burst_probability < 0.0 or args.burst_probability > 1.0):
    ...
    try:
        if args.profile:
            ...
            if args.burst_rate_multiplier is not None:
                timing_model_params['burst_rate_multiplier'] = args.burst_rate_multiplier
        ...
        if monitor_type == "enhanced":
            ...
            try:
                ...
                print("Warning: Enhanced monitoring requires psutil. Install with: pip install nanorunner[enhanced]")
