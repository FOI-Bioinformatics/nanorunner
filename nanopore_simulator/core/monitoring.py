...
class SimulationMetrics:
    ...
    def estimate_eta(self):
        ...
        if len(self.throughput_history) >= 3:
            recent_throughputs = list(self.throughput_history)[-10:]  # Last 10 measurements
            ...
            if len(recent_throughputs) >= 5:
                ...
                if trend_ratio > 1.05:  # 5% improvement
                    ...
                    predicted_throughput = avg_recent * min(trend_ratio, 1.2)  # Cap at 20% improvement
                elif trend_ratio < 0.95:  # 5% degradation
                    self.eta_trend = "degrading"
                    # Use conservative prediction
                    predicted_throughput = avg_recent * max(trend_ratio, 0.8)  # Cap at 20% degradation
                else:
                    self.eta_trend = "stable"
                    predicted_throughput = avg_recent
                
                self.eta_seconds = remaining_files / predicted_throughput if predicted_throughput > 0 else simple_eta
                
                # Confidence based on consistency of recent measurements
                variance = sum((x - avg_recent) ** 2 for x in recent_throughputs) / len(recent_throughputs)
                coefficient_of_variation = (variance ** 0.5) / avg_recent if avg_recent > 0 else 1.0
                self.confidence_score = max(0.0, min(1.0, 1.0 - coefficient_of_variation))
            else:
...
class ProgressDisplay:
    ...
    @staticmethod
    def format_progress_line(metrics: SimulationMetrics, include_bar: bool = True, 
                           show_resources: bool = False) -> str:
...
class ResourceMonitor:
    ...
    def get_current_metrics(self) -> ResourceMetrics:
        ...
        try:
            ...
            logging.getLogger(__name__).warning(f"Error collecting resource metrics: {e}")
            return ResourceMetrics()
...
class SignalHandler:
    ...
    def _print_shutdown_summary(self, metrics: SimulationMetrics):
        ...
        if metrics.total_bytes_processed > 0:
            print(f"Data processed: {ProgressDisplay.format_bytes(metrics.total_bytes_processed)}")
...
class ProgressMonitor:
    ...
    def __init__(self, total_files: int, update_interval: float = 1.0, 
                 display_callback: Optional[Callable[[SimulationMetrics], None]] = None,
                 enable_resources: bool = True, enable_checkpoint: bool = True):
        ...
        self.checkpoint_file = Path("simulation_checkpoint.json") if enable_checkpoint else None
            ...
            with self._lock:
                ...
                if self.resource_monitor:
                    ...
                    self.metrics.total_disk_io_mb = (
                        resource_metrics.disk_io_read_mb + resource_metrics.disk_io_write_mb
                    )
                
                # Update performance metrics
                self.metrics.update_throughput()
                self.metrics.add_throughput_sample(self.metrics.throughput_files_per_sec)
                self.metrics.estimate_eta()
    ...
    def _check_performance_warnings(self):
        ...
        if self.metrics.resource_metrics.memory_percent > 85:
            warnings.append(f"High memory usage: {self.metrics.resource_metrics.memory_percent:.1f}%")
        
        # High CPU usage sustained
        if len(self.metrics.resource_history) >= 5:
            recent_cpu = [r.cpu_percent for r in list(self.metrics.resource_history)[-5:]]
            avg_cpu = sum(recent_cpu) / len(recent_cpu)
            if avg_cpu > 90:
                warnings.append(f"Sustained high CPU usage: {avg_cpu:.1f}%")
        
        # Low throughput
        if (len(self.metrics.throughput_history) >= 10 and 
            self.metrics.throughput_files_per_sec < 0.1 and 
            self.metrics.files_processed > 10):
            warnings.append(f"Low throughput: {self.metrics.throughput_files_per_sec:.2f} files/sec")
        
        # High disk usage
        if self.metrics.resource_metrics.disk_usage_percent > 90:
            warnings.append(f"Low disk space: {self.metrics.resource_metrics.disk_usage_percent:.1f}% used")
    ...
    def _load_checkpoint(self):
        ...
        try:
            ...
            if 'metrics' in checkpoint_data:
                ...
                self.metrics.total_bytes_processed = saved_metrics.get('total_bytes_processed', 0)
                
            self.logger.info(f"Resumed from checkpoint: {self.metrics.files_processed} files processed")
    ...
    def _default_display(self, metrics: SimulationMetrics):
        ...
        if metrics.files_total > 0:
            ...
            progress_line = ProgressDisplay.format_progress_line(metrics, show_resources=show_resources)
    ...
    def _print_final_summary(self, metrics: SimulationMetrics):
        ...
        if len(metrics.throughput_history) >= 5:
            ...
            if max_tp > min_tp * 1.5:
                print(f"📊 Throughput varied from {min_tp:.1f} to {max_tp:.1f} files/sec")
...
class DetailedProgressMonitor(ProgressMonitor):
    ...
    def end_batch(self, start_time: float):
        ...
        batch_info = {
            ...
            'files_in_batch': len([f for f in self.file_details 
    ...
    def _log_final_summary(self, metrics: SimulationMetrics):
        ...
        self.detailed_logger.info(f"Data processed: {ProgressDisplay.format_bytes(metrics.total_bytes_processed)}")
        self.detailed_logger.info(f"Average throughput: {metrics.throughput_files_per_sec:.1f} files/sec")
        self.detailed_logger.info(f"Average file size: {ProgressDisplay.format_bytes(int(metrics.average_file_size))}")
        
        if metrics.errors_encountered > 0:
            self.detailed_logger.warning(f"Errors encountered: {metrics.errors_encountered}")
        
        # Timing breakdown
        self.detailed_logger.info("Timing breakdown:")
        for category, duration in metrics.timing_breakdown.items():
            percentage = (duration / metrics.elapsed_time) * 100 if metrics.elapsed_time > 0 else 0
            self.detailed_logger.info(f"  {category}: {ProgressDisplay.format_time(duration)} ({percentage:.1f}%)")
