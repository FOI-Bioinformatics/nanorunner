...
class PipelineRequirements:
    ...
    required_structure: Optional[str] = None  # "singleplex", "multiplex", or None for flexible
...
class PipelineAdapter(ABC):
    ...
    def __init__(self, requirements: PipelineRequirements):
        ...
        if requirements.barcode_patterns:
            self._barcode_patterns = [re.compile(pattern) for pattern in requirements.barcode_patterns]
        else:
    ...
    def _glob_to_regex(self, glob_pattern: str) -> str:
        ...
        if glob_pattern.startswith("**/"):
            ...
            regex_pattern = r".*" + re.escape(file_pattern).replace(r"\*", ".*").replace(r"\?", ".")
        else:
            # Simple glob pattern
            regex_pattern = re.escape(glob_pattern).replace(r"\*", ".*").replace(r"\?", ".")
    ...
    def get_validation_report(self, target_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            report['files_found'] = [str(f.relative_to(target_dir)) for f in found_files]
            ...
            if self.requirements.metadata_files:
                for metadata_file in self.requirements.metadata_files:
                    ...
                    if not metadata_path.exists():
                        ...
                        report['warnings'].append(f"Missing metadata file: {metadata_file}")
            
            # Overall validation
            report['valid'] = (report['structure_valid'] and 
                             len(report['files_found']) > 0 and 
...
class MiniknifeAdapter(PipelineAdapter):
    ...
    def validate_structure(self, target_dir: Path) -> bool:
        ...
        barcode_dirs = [d for d in target_dir.iterdir() 
...
class AdapterManager:
    ...
    def list_adapters(self) -> Dict[str, str]:
        ...
        return {name: adapter.requirements.description 
                for name, adapter in self.adapters.items()}
    
    def validate_for_pipeline(self, pipeline_name: str, target_dir: Path) -> Dict[str, Any]:
