...
class NanoporeSimulator:
    ...
    def __init__(self, config: SimulationConfig, enable_monitoring: bool = True,
        ...
        if config.parallel_processing:
            ...
            self.logger.info(f"Parallel processing enabled with {config.worker_count} workers")
    ...
    def run_simulation(self):
        ...
        if self.enable_monitoring:
            ...
            total_batches = (len(file_manifest) + self.config.batch_size - 1) // self.config.batch_size
            self.progress_monitor.set_batch_count(total_batches)
            self.progress_monitor.start()
            
            self.logger.info(f"Enhanced monitoring initialized: {self.monitor_type} mode")
    ...
    def _execute_simulation(self, file_manifest: List[Dict], structure: str):
        ...
        for i, batch_start in enumerate(range(0, total_files, self.config.batch_size)):
            ...
            if self.progress_monitor and self.progress_monitor.should_stop():
                self.logger.info("Shutdown signal received, stopping simulation gracefully")
                break
            ...
            batch_start_time = self.progress_monitor.start_batch() if self.progress_monitor else time.time()
            ...
            try:
                ...
                if self.progress_monitor:
                    self.progress_monitor.record_error(f"critical_error: {type(e).__name__}")
                ...
                if self.progress_monitor:
                    self.progress_monitor.record_error(f"batch_processing: {type(e).__name__}")
            ...
            self.logger.info(f"Batch {batch_num} completed in {batch_duration:.2f}s "
                           f"({throughput:.1f} files/sec)")
            
            # Wait for next batch (except for last batch)
            if batch_end < total_files:
                actual_interval = self._calculate_interval()
                self.logger.info(f"Waiting {actual_interval:.2f} seconds before next batch...")
    ...
    def _process_file(self, file_info: Dict[str, Union[Path, str]]):
        ...
        try:
            ...
            if self.progress_monitor:
                self.progress_monitor.record_file_processed(target, total_operation_duration)
            ...
            if self.monitor_type == "detailed":
                if barcode:
                    self.logger.info(f"{operation}: {source.name} -> {barcode}/{target.name} ({total_operation_duration:.3f}s)")
                else:
                    self.logger.info(f"{operation}: {source.name} -> {target.name} ({total_operation_duration:.3f}s)")
                
        except Exception as e:
            # Record error in monitoring
            if self.progress_monitor:
                self.progress_monitor.record_error(f"file_operation: {type(e).__name__}")
