...
def invalid_config_params(tmp_path):
    ...
    return {
        ...
        "invalid_structure": {
            **base_params,
            "force_structure": "invalid_structure"
        }
    }
...
class ConfigBuilder:
    ...
    def with_parallel_processing(self, enabled: bool = True, 
                               worker_count: int = 4) -> "ConfigBuilder":
...
def validate_config_parameters(config: SimulationConfig) -> Dict[str, Any]:
    ...
    if config.interval < 0.1:
        results["warnings"].append(
            "Very short interval may cause high system load"
        )
...
def create_config_variants(base_config: SimulationConfig) -> Dict[str, SimulationConfig]:
