...
def create_test_fastq_content(
    reads: int = 10, read_length: int = 1000, quality_score: str = "I"
) -> str:
...
def create_realistic_fastq(
    file_path: Path, reads: int = 100, compressed: bool = False
) -> Path:
...
class DataTestManager:
    ...
    def create_temp_structure(
        self, structure_type: str = "singleplex", file_count: int = 5
    ) -> Path:
...
def create_performance_dataset(size: str = "medium") -> Path:
    ...
    if size not in size_configs:
        raise ValueError(
            f"Unknown size: {size}. Choose from {list(size_configs.keys())}"
        )
...
def _validate_singleplex_structure(path: Path) -> bool:
    ...
    return len(fastq_files) > 0 and not any(
        d.is_dir() for d in path.iterdir() if d.name.startswith("barcode")
    )
...
def _validate_multiplex_structure(path: Path) -> bool:
    ...
    barcode_dirs = [
        ...
        if d.is_dir()
        and (
            d.name.startswith("barcode")
            or d.name.startswith("BC")
            or d.name == "unclassified"
        )
    ]
...
def _validate_mixed_structure(path: Path) -> bool:
    ...
    barcode_dirs = [
        d for d in path.iterdir() if d.is_dir() and d.name.startswith("barcode")
    ]
