...
class RealisticDataGenerator:
    ...
    def generate_realistic_fastq_read(read_id: str, length: int = None) -> str:
        ...
        if length is None:
            ...
            length = random.choices(
                [100, 200, 500, 1000, 2000], weights=[40, 30, 20, 8, 2]
            )[0]
    ...
    @staticmethod
    def create_realistic_fastq_file(
        filepath: Path, num_reads: int = 1000, compress: bool = False
    ) -> Dict[str, any]:
        ...
        for i in range(num_reads):
            ...
            read_length = random.choices(
                [500, 1000, 2000, 5000, 10000, 20000], weights=[25, 30, 20, 15, 8, 2]
            )[0]

            read = RealisticDataGenerator.generate_realistic_fastq_read(
                read_id, read_length
            )
            reads.append(read)
    ...
    @staticmethod
    def create_realistic_pod5_file(
        filepath: Path, num_reads: int = 500
    ) -> Dict[str, any]:
...
class RealisticSequencingScenarios:
    ...
    @staticmethod
    def create_minion_run(
        base_dir: Path, run_name: str = "minion_run"
    ) -> Dict[str, any]:
        ...
        for i in range(file_count):
            ...
            if random.random() < 0.7:  # 70% FASTQ
                ...
                metadata = RealisticDataGenerator.create_realistic_pod5_file(
                    filepath, reads
                )
                total_reads += reads
                files_created.append((filepath, metadata))
    ...
    @staticmethod
    def create_promethion_run(
        base_dir: Path, run_name: str = "promethion_run"
    ) -> Dict[str, any]:
    ...
    @staticmethod
    def create_multiplex_barcoded_run(
        base_dir: Path, num_barcodes: int = 12
    ) -> Dict[str, any]:
...
def multiplex_run_fixture():
    ...
    scenario = RealisticSequencingScenarios.create_multiplex_barcoded_run(
        temp_path, num_barcodes=3
    )
