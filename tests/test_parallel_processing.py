...
class TestParallelPerformance:
    ...
    def test_parallel_vs_sequential_performance(self, large_file_setup):
        ...
        try:
            ...
            assert speed_ratio > 0.8, f"Parallel processing should be competitive (ratio: {speed_ratio})"
            
            # Verify all files were processed correctly
            for file_info in file_manifest:
                assert file_info['target'].exists()
                assert file_info['target'].stat().st_size == file_info['source'].stat().st_size
        finally:
