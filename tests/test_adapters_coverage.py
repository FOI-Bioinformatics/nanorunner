...
from nanopore_simulator.core.adapters import (
    ...
    KrackenAdapter, MiniknifeAdapter, GenericAdapter,
    AdapterManager, get_available_adapters, validate_for_pipeline,
    get_compatible_pipelines, get_pipeline_adapter
)
...
class TestKrackenAdapterEdgeCases:
    ...
    def test_kracken_adapter_error_conditions(self):
        ...
        with patch.object(adapter, 'validate_structure', side_effect=PermissionError("Access denied")):
            report = adapter.get_validation_report(target_dir)
...
class TestGenericAdapterEdgeCases:
    ...
    def test_generic_adapter_with_custom_validation_rules(self):
        ...
        # Create files
        for i in range(3):
            (target_dir / f"sample_{i}.fastq").write_text(f"@read{i}\\nACGT\\n+\\nIIII\\n")
    ...
    def test_generic_adapter_validation_exception(self):
        ...
        with patch.object(adapter, 'validate_structure', side_effect=RuntimeError("Validation failed")):
            report = adapter.get_validation_report(target_dir)
...
class TestAdapterIntegrationEdgeCases:
    ...
    def test_adapter_performance_with_large_file_counts(self):
        ...
        # Create many files
        for i in range(100):
            (target_dir / f"file_{i:03d}.fastq").write_text(f"@read{i}\\nACGT\\n+\\nIIII\\n")
...
class TestAdapterErrorRecovery:
    ...
    def test_adapter_graceful_degradation(self):
        ...
        with patch('builtins.list', side_effect=lambda x: list(x)[:10]):  # Limit to 10 items
            report = adapter.get_validation_report(target_dir)
