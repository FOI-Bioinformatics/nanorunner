...
class TestPerformance:
    ...
    def test_large_file_count_performance(self):
        ...
        print(f"Processed {num_files} files in {processing_time:.2f}s ({throughput:.1f} files/sec)")
    ...
    def test_deep_barcode_hierarchy_performance(self):
        ...
        for i in range(1, num_barcodes + 1):
            ...
            for j in range(5):
                (barcode_dir / f"reads_{j}.fastq").write_text(f"@read_{i}_{j}\nACGT\n+\nIIII\n")
        ...
        total_files = sum(len(list(bc_dir.glob("*.fastq"))) for bc_dir in target_barcodes)
        assert total_files == num_barcodes * 5
        
        processing_time = end_time - start_time
        print(f"Processed {num_barcodes} barcode directories ({total_files} files) in {processing_time:.2f}s")
    ...
    def test_memory_usage_large_manifest(self):
        ...
        print(f"Manifest memory usage: {manifest_size} bytes ({size_per_item:.1f} bytes/item)")
    ...
    def test_symlink_vs_copy_performance(self):
        ...
        num_files = 100
        for i in range(num_files):
            (source_dir / f"test_{i:03d}.fastq").write_text("@read\n" + "A" * 1000 + "\n+\n" + "I" * 1000 + "\n")
    ...
    def test_batch_size_impact_on_performance(self):
        ...
        num_files = 200
        for i in range(num_files):
            (source_dir / f"batch_test_{i:03d}.fastq").write_text("@read\nACGT\n+\nIIII\n")
