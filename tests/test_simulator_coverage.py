...
class TestSimulatorEdgeCases:
    ...
    def create_sample_files(self, count=5):
        ...
        for i in range(count):
            (source_dir / f"sample_{i:03d}.fastq").write_text(
                f"@read{i}\\nACGT\\n+\\nIIII\\n"
            )
    ...
    def test_simulator_file_discovery_errors(self):
        ...
        with patch(
            "nanopore_simulator.core.detector.FileStructureDetector._find_sequencing_files",
    ...
    def test_simulator_parallel_processing_errors(self):
        ...
        with patch(
            "shutil.copy2",
            side_effect=PermissionError("Critical parallel processing error"),
        ):
            with pytest.raises(
                PermissionError, match="Critical parallel processing error"
            ):
                simulator.run_simulation()
    ...
    def test_simulator_directory_structure_creation_errors(self):
        ...
        for i in range(3):
            ...
            barcode_dir.mkdir()
            (barcode_dir / f"reads_{i}.fastq").write_text(
                f"@read{i}\\nACGT\\n+\\nIIII\\n"
            )
    ...
    def test_simulator_progress_monitoring_edge_cases(self):
        ...
        simulator = NanoporeSimulator(
            config, enable_monitoring=True, monitor_type="default"
        )
        simulator.run_simulation()
        ...
        if hasattr(simulator, "pause_simulation"):
            ...
            config2 = SimulationConfig(
                source_dir=source_dir2, target_dir=target_dir2, interval=0.01
            )
    ...
    def test_simulator_monitoring_callback_errors(self):
        ...
        with patch(
            "nanopore_simulator.core.monitoring.create_progress_monitor"
        ) as mock_create:
    ...
    def test_simulator_adaptive_timing_edge_cases(self):
        ...
        # Disable monitoring to avoid signal handler interference in tests
