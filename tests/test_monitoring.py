...
from nanopore_simulator.core.monitoring import (
    SimulationMetrics, ProgressDisplay, ProgressMonitor,
    DetailedProgressMonitor, create_progress_monitor
)
...
class TestSimulationMetrics:
    ...
    def test_update_throughput(self):
        ...
        assert abs(metrics.throughput_files_per_sec - 2.0) < 0.01  # ~20 files / 10 seconds
        assert abs(metrics.throughput_bytes_per_sec - 200.0) < 0.01  # ~2000 bytes / 10 seconds
...
class TestProgressMonitorFactory:
    ...
    def test_create_monitor_with_custom_params(self):
        ...
        monitor = create_progress_monitor(15, monitor_type="default", 
                                        update_interval=0.5)
...
class TestProgressMonitorIntegration:
    ...
    def test_error_handling_during_monitoring(self):
        ...
        monitor = ProgressMonitor(total_files=5, update_interval=0.05,
                                display_callback=faulty_display)
