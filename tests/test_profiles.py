...
from nanopore_simulator.core.profiles import (
    ProfileDefinition, ProfileManager, BUILTIN_PROFILES,
    get_available_profiles, create_config_from_profile, 
    get_profile_recommendations, validate_profile_name
)
...
class TestBuiltinProfiles:
    ...
    def test_builtin_profiles_exist(self):
        ...
        expected_profiles = [
            "rapid_sequencing", "accurate_mode", "development_testing",
            "long_read_nanopore", "high_throughput", "adaptive_learning",
            "legacy_random", "minion_simulation", "promethion_simulation"
        ]
...
class TestProfileManager:
    ...
    def test_create_config_unknown_profile(self, temp_dirs):
        ...
        with pytest.raises(ValueError, match="Profile 'unknown' not found"):
            manager.create_config_from_profile(
                "unknown", source_dir, target_dir
            )
...
class TestProfileRecommendations:
    ...
    def test_recommendations_for_large_dataset(self):
        ...
        assert "high_throughput" in recommendations or "promethion_simulation" in recommendations
        assert len(recommendations) <= 5
...
class TestProfileIntegration:
    ...
    def test_profile_config_validation(self, temp_dirs):
        ...
        for profile_name in BUILTIN_PROFILES.keys():
            config = create_config_from_profile(
                profile_name, source_dir, target_dir
            )
    ...
    def test_legacy_compatibility(self, temp_dirs):
        ...
        config = create_config_from_profile(
            "legacy_random", source_dir, target_dir
        )
