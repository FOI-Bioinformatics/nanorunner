...
class TestCLIAdditionalCoverage:
    ...
    def test_validate_pipeline_with_warnings_and_errors(self, capsys):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('nanopore_simulator.cli.main.validate_for_pipeline') as mock_validate:
                mock_validate.return_value = {
                    'valid': False,
                    'files_found': ['file1.fastq', 'file2.fastq', 'file3.fastq', 'file4.fastq', 'file5.fastq', 'file6.fastq'],
    ...
    def test_cli_argument_validation_errors(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(sys, 'argv', [
                'nanorunner', str(source_dir), str(target_dir),
                '--random-factor', '2.0'  # Invalid, must be 0.0-1.0
            ]):
            ...
            with patch.object(sys, 'argv', [
                'nanorunner', str(source_dir), str(target_dir),
                '--burst-probability', '1.5'  # Invalid, must be 0.0-1.0
            ]):
                with pytest.raises(SystemExit) as exc_info:
                    main()
                assert exc_info.value.code == 2
            ...
            with patch.object(sys, 'argv', [
                'nanorunner', str(source_dir), str(target_dir),
                '--burst-rate-multiplier', '-1.0'  # Invalid, must be positive
            ]):
                with pytest.raises(SystemExit) as exc_info:
                    main()
                assert exc_info.value.code == 2
    ...
    def test_cli_nonexistent_source_directory(self):
        ...
        with patch.object(sys, 'argv', [
            'nanorunner', '/nonexistent/source', '/some/target'
        ]):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 2
    ...
    def test_cli_profile_configuration_building(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(sys, 'argv', [
                ...
                with patch('nanopore_simulator.cli.main.validate_profile_name', return_value=True):
                    with patch('nanopore_simulator.cli.main.create_config_from_profile') as mock_create:
                        with patch('nanopore_simulator.cli.main.NanoporeSimulator') as mock_sim_class:
                            mock_sim = MagicMock()
    ...
    def test_cli_standard_configuration_with_timing_params(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(sys, 'argv', [
                ...
                with patch('nanopore_simulator.cli.main.SimulationConfig') as mock_config:
                    with patch('nanopore_simulator.cli.main.NanoporeSimulator') as mock_sim_class:
                        mock_sim = MagicMock()
                        mock_sim_class.return_value = mock_sim
            ...
            with patch.object(sys, 'argv', [
                ...
                with patch('builtins.__import__', side_effect=mock_import):
                    with patch('nanopore_simulator.cli.main.NanoporeSimulator') as mock_sim_class:
                        ...
                        captured = capsys.readouterr()
                        assert "Warning: Enhanced monitoring requires psutil" in captured.out
                        assert "Falling back to detailed monitoring mode" in captured.out
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(sys, 'argv', [
                ...
                with patch('nanopore_simulator.cli.main.NanoporeSimulator') as mock_sim_class:
                    ...
                    captured = capsys.readouterr()
                    assert "Enhanced monitoring active. Interactive controls:" in captured.out
            ...
            with patch.object(sys, 'argv', [
                ...
                with patch('nanopore_simulator.cli.main.NanoporeSimulator') as mock_sim_class:
                    with patch('nanopore_simulator.cli.main.validate_for_pipeline') as mock_validate:
                        ...
                        captured = capsys.readouterr()
                        assert "Validating output for nanometanf pipeline" in captured.out
                        assert "✓ Output is compatible with nanometanf pipeline" in captured.out
            ...
            with patch.object(sys, 'argv', [
                ...
                with patch('nanopore_simulator.cli.main.NanoporeSimulator') as mock_sim_class:
                    with patch('nanopore_simulator.cli.main.validate_for_pipeline') as mock_validate:
                        ...
                        mock_validate.return_value = {
                            'valid': False,
                            'warnings': ['Missing required files', 'Incorrect structure']
                        }
                        ...
                        captured = capsys.readouterr()
                        assert "✗ Output may not be compatible with kraken pipeline" in captured.out
