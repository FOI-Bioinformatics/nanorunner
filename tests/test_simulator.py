...
class TestNanoporeSimulator:
    ...
    def test_execute_simulation_timing(self, mock_sleep):
        ...
        # Create manifest
        manifest = [
            {'source': self.source_dir / "file1.fastq", 'target': self.target_dir / "file1.fastq", 'barcode': None},
            {'source': self.source_dir / "file2.fastq", 'target': self.target_dir / "file2.fastq", 'barcode': None},
            {'source': self.source_dir / "file3.fastq", 'target': self.target_dir / "file3.fastq", 'barcode': None},
        ]
    ...
    def test_run_simulation_multiplex(self):
        ...
        # Check content
        assert (self.target_dir / "barcode01" / "reads.fastq").read_text() == "bc01_content"
        assert (self.target_dir / "unclassified" / "unassigned.fastq").read_text() == "unclass_content"
