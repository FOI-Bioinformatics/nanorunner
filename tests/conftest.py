...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", 
        "coverage: marks tests specifically for coverage improvement"
    )
    
    # Pipeline-specific markers
    config.addinivalue_line(
        "markers",
        "nanometanf: marks tests specific to nanometanf pipeline integration"
    )
    config.addinivalue_line(
        "markers",
        "kraken: marks tests specific to Kraken pipeline integration"
    )
    config.addinivalue_line(
        "markers",
        "miniknife: marks tests specific to miniknife pipeline integration"
    )
...
def performance_context():
    ...
    class PerformanceContext:
        ...
        def assert_throughput_over(self, items_per_second: float, 
                                 item_count: int, name: str = "default"):
...
def test_validator():
    ...
    class TestValidator:
        def validate_file_structure(self, path: Path, 
        ...
        def validate_timing_accuracy(self, expected: float, actual: float, 
        ...
        def validate_performance_metrics(self, metrics: Dict[str, Any], 
...
def thread_test_utilities():
    ...
    class ThreadTestUtilities:
        ...
        def run_concurrent(self, functions: List[callable], 
        ...
        def assert_thread_safety(self, shared_resource, operations: List[callable],
                               thread_count: int = 5, iterations: int = 10):
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    if hasattr(terminalreporter, 'stats'):
        ...
        unit_tests = sum(1 for item in stats.get('passed', []) 
                        if 'unit' in str(item))
        integration_tests = sum(1 for item in stats.get('passed', []) 
                               if 'integration' in str(item))
        coverage_tests = sum(1 for item in stats.get('passed', []) 
                            if 'coverage' in str(item))
