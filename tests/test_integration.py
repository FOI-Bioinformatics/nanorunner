...
from nanopore_simulator import SimulationConfig, NanoporeSimulator, FileStructureDetector
...
class TestIntegrationWorkflows:
    ...
    def create_sample_data(self, structure_type="singleplex"):
        ...
        if structure_type == "singleplex":
            ...
            for barcode_num in ["01", "02", "12"]:
                ...
                barcode_dir.mkdir()
                (barcode_dir / f"reads_{barcode_num}.fastq").write_text(f"@read_bc{barcode_num}\nACGT\n+\nIIII\n")
                (barcode_dir / f"additional_{barcode_num}.fastq.gz").write_text(f"compressed_bc{barcode_num}")
            
            # Add unclassified directory
            unclass_dir = source_dir / "unclassified"
            unclass_dir.mkdir()
            (unclass_dir / "unassigned.fastq").write_text("@unassigned_read\nNNNN\n+\n!!!!\n")
    ...
    def test_console_script_integration(self):
        ...
        cmd = [
            sys.executable, "-m", "nanopore_simulator.cli.main",
            str(source_dir),
            str(target_dir),
            "--interval", "0.1",
            "--batch-size", "3"
    ...
    def test_forced_structure_override(self):
        ...
        source_dir = self.create_sample_data("mixed")  # Has both direct files and barcode dirs
    ...
    def test_large_batch_processing(self):
        ...
        for i in range(num_files):
            (source_dir / f"file_{i:03d}.fastq").write_text(f"@read{i}\nACGT\n+\nIIII\n")
    ...
    def test_mixed_file_types_workflow(self):
        ...
        sequencing_files = ["reads.fastq", "reads.fq", "reads.fastq.gz", "reads.fq.gz", "signals.pod5"]
    ...
    def test_nested_barcode_structure(self):
        ...
        barcode_combinations = [
            ("barcode", "01"), ("barcode", "12"), ("barcode", "96"),
            ("BC", "02"), ("BC", "13"), ("BC", "97"),  # Different numbers to avoid case conflicts
            ("bc", "03"), ("bc", "14"), ("bc", "98")   # Different numbers to avoid case conflicts
        ]
        ...
        for bc_pattern, num in barcode_combinations:
            ...
            (barcode_dir / "reads_part1.fastq").write_text(f"@{bc_pattern}{num}_1\nACGT\n+\nIIII\n")
            (barcode_dir / "reads_part2.fastq.gz").write_text(f"compressed_{bc_pattern}{num}")
        ...
        total_files = sum(len(list(d.glob("*"))) for d in target_dir.iterdir() if d.is_dir())
    ...
    def test_error_recovery_and_partial_completion(self):
        ...
        with patch.object(simulator, '_process_file', side_effect=[
